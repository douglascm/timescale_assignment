...
Google Cloud has an "all-in-one" solution offered within Google Cloud: Cloud Composer, which would reduce devops time and allow mroe focus on the development. Cloud Composer is a Kubernetes cluster consisting of a Flask web server for Airflow (App Engine Flex), a scheduler to distribute tasks to workers, a Redis queue for scheduling tasks, a database for holding DAG data, as well as workers. Selecting the most appropriate tools as well as integrating with Timescale would be the goal here. If the pre-existing environment in the company was using custom built servers/clusters, then integrating with its existing systems would be simpler.
